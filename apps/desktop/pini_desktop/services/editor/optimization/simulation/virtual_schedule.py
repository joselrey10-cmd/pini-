...
class VirtualSession:
    """In-memory representation of a schedule session.

    The class deliberately keeps only the fields needed by the optimizer. Extra
    data can be carried in ``metadata`` without coupling the simulator to the
    database layer or Qt views.
    """
...
class VirtualSchedule:
    """Complete schedule copy that can be modified safely in memory."""
